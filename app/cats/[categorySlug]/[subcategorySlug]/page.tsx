...
export default async function CatSubcategoryPage({ params, searchParams }: SubcategoryPageProps) {
  const { categorySlug, subcategorySlug } = await params
  const resolvedSearchParams = await searchParams

  console.log("[CatSubcategoryPage] Loading subcategory:", subcategorySlug)
  ...
  try {
    ...
    console.error("[CatSubcategoryPage] Error fetching data:", err)
    if (!subcategory) {
    ...
    try {
      ...
      console.error("[CatSubcategoryPage] Failed to fetch brands:", bErr)
      brands = []
    }
  }
  ...
  if (subcategory.parentId && subcategory.parentName && subcategory.parentSlug) {
    breadcrumbItems.push({
      ...
      href: `/cats/${subcategory.parentSlug}`,
    })
  }
