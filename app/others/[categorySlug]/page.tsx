...
export default async function OtherCategoryPage({ params, searchParams }: CategoryPageProps) {
  const { categorySlug } = await params
  const resolvedSearchParams = await searchParams

  console.log("[OtherCategoryPage] Loading category:", categorySlug)
  ...
  try {
    ...
    console.error("[OtherCategoryPage] Error fetching data:", err)
    if (!category) {
    ...
    try {
      ...
      console.error("[OtherCategoryPage] Failed to fetch brands:", bErr)
      brands = []
    }
  }
  ...
  if (category.parentId && category.parentName && category.parentSlug) {
    breadcrumbItems.push({
      ...
      href: `/others/${category.parentSlug}`,
    })
  }
