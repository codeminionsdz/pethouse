...
export async function generateMetadata({ params }: SubcategoryPageProps): Promise<Metadata> {
  ...
  if (!subcategory) {
    return { title: "Category Not Found | PETHOUSE" }
  }

  return {
    title: `${subcategory.name} - ${ANIMAL_NAME} | PETHOUSE`,
    description: subcategory.description,
  }
}

export default async function BirdSubcategoryPage({ params, searchParams }: SubcategoryPageProps) {
  const { categorySlug, subcategorySlug } = await params
  const resolvedSearchParams = await searchParams

  console.log("[BirdSubcategoryPage] Loading subcategory:", subcategorySlug)
  ...
  try {
    ...
    console.error("[BirdSubcategoryPage] Error fetching data:", err)
    if (!subcategory) {
    ...
    try {
      ...
      console.error("[BirdSubcategoryPage] Failed to fetch brands:", bErr)
      brands = []
    }
  }
