...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  return {
    title: `${category.name} - ${ANIMAL_NAME} | PETHOUSE`,
    description: category.description,
  }
}

export default async function BirdCategoryPage({ params, searchParams }: CategoryPageProps) {
  const { categorySlug } = await params
  const resolvedSearchParams = await searchParams

  console.log("[BirdCategoryPage] Loading category:", categorySlug)
  ...
  try {
    ...
    console.error("[BirdCategoryPage] Error fetching data:", err)
    if (!category) {
    ...
    try {
      ...
      console.error("[BirdCategoryPage] Failed to fetch brands:", bErr)
      brands = []
    }
  }
  ...
  if (category.parentId && category.parentName && category.parentSlug) {
    breadcrumbItems.push({
      ...
      href: `/birds/${category.parentSlug}`,
    })
  }
