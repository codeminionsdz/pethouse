...
export async function getProductBySlug(slug: string): Promise<Product | null> {
  ...
  // Build slug candidates to handle encoded URLs and space/hyphen differences
  ...
  const candidates = Array.from(
    new Set([
      ...
      decodedSlug.replace(/-/g, " ").toLowerCase().trim(),
    ].filter(Boolean)),
  )
  ...
  if (error) {
    console.error("Error fetching product by slug:", error?.message || JSON.stringify(error), "slug:", originalSlug)
    return null
  }

  let product = data?.[0]

  // Fallback: support opening product by name from URL segment (e.g. /products/Toy)
  if (!product) {
    ...
    for (const nameCandidate of nameCandidates) {
      ...
      if (nameError) {
        console.error("Error fetching product by name fallback:", nameError?.message || JSON.stringify(nameError), "name:", nameCandidate)
        continue
      }
  ...
  if (!product) {
    console.log("No product found for slug:", originalSlug, "candidates:", candidates)
    return null
  }
